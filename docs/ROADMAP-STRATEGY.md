...
| ~~MCP Prompt 模板 (3 个 prompt)~~ | P0 | ❌ 已移除（经验主义，无 agent 消费） |
...
| 安全回滚（checkpoint/undo） | 用户反馈或竞品验证（veil 已主打） | 文件系统快照 + 命令级回滚；属新功能，非缺陷修复，发布后评估 |
| Opt-in 遥测（激活/留存指标） | 发布后有真实用户、需要测量 D1/D7 留存 | 隐私默认 + 源码可审计；当前刻意不采集（`telemetry.enabled` 已移除） |
...
| 行业 typed tool 收敛（agent 少用 shell） | **中** | 锚定 typed tool 难覆盖的执行层（构建/测试/运维/系统）；Python 执行层解析 + 修正闭环可量化见 Phase 4 计划 |
| 性能回归 | **低** | 脏标记 + 定时刷新机制，403 测试覆盖 |
| 数据一致性 | **低** | merge_enriched_events TOCTOU 修复，原子操作 |
| Parser 覆盖率不足 | **中** | 37 parser 全量审计，持续优化；37 为软上限，新 parser 须用户反馈驱动（不追广度） |
| 异步任务 enrichment 不完整 | **低** | detected_tool 传递已修复；context enrichment 正常（hint 合成已移除，修复交由 LLM） |
