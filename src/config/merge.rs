...
pub fn apply_env(cfg: &mut Config) {
    ...
    if let Ok(v) = std::env::var("ARSHY_DAEMON_IDLE_TIMEOUT_SECS") {
        if let Ok(n) = v.parse::<u64>() {
            // 0 disables idle-exit; otherwise clamp to a sane minimum.
            cfg.daemon.idle_timeout_secs = if n == 0 { 0 } else { n.max(30) };
        }
    }
    // Store
    if let Ok(v) = std::env::var("ARSHY_STORE_STORE_DIR") {
        cfg.store.store_dir = PathBuf::from(v);
    }
...
pub fn apply_cli(cfg: &mut Config, cli: &CliOverrides) {
    ...
    if let Some(ref v) = cli.store_dir {
        cfg.store.store_dir = v.clone();
    }
}
...
mod tests {
    ...
    fn merge_partial_daemon_all_fields() {
        ...
        assert_eq!(merged.daemon.idle_timeout_secs, 300);
    }
    ...
    fn merge_partial_parser_section() {
        let cfg = Config::default();
        let partial = PartialConfig {
            parser: Some(PartialParserConfig { dirs: None, hot_reload: Some(false) }),
            ..Default::default()
        };
        let merged = merge_partial(cfg, partial);
