...
pub fn run_wrapper(exe_name: &str, args: Vec<String>) -> Result<()> {
    ...
    let intercept = should_intercept(
        &args,
...
fn should_intercept(
    ...
    if has_tty_conflict || arg_contains_arshy {
        return false;
    }

    is_agent_env || is_known_agent_parent || is_ambiguous_agent_parent
}
...
mod tests {
    ...
    fn test_should_intercept_interactive_no_c() {
        ...
        assert!(!should_intercept(&args, env_check, &None, true));
    }
    ...
    fn test_should_intercept_non_interactive_no_agent() {
        ...
        let env_check = |_key: &str| false;
        assert!(!should_intercept(&args, env_check, &None, true));
        assert!(!should_intercept(&args, env_check, &Some("bash".to_string()), true));
    }
    ...
    fn test_should_intercept_agent_env() {
        ...
        assert!(should_intercept(&args, env_check_arshy, &None, true));

        let env_check_claude = |key: &str| key == "CLAUDE_CODE";
        assert!(should_intercept(&args, env_check_claude, &None, true));
    }
    ...
    fn test_should_intercept_agent_parent() {
        ...
        assert!(should_intercept(&args, env_check, &Some("Cursor".to_string()), true));
        ...
        assert!(should_intercept(&args, env_check, &Some("claude-code".to_string()), true));
        assert!(should_intercept(&args, env_check, &Some("antigravity".to_string()), true));
        assert!(should_intercept(&args, env_check, &Some("gemini-agent".to_string()), true));
        assert!(should_intercept(&args, env_check, &Some("agy".to_string()), true));
    }
    ...
    fn test_should_intercept_agent_but_workspace_not_configured() {
        ...
        let args = vec!["sh".to_string(), "-c".to_string(), "cargo build".to_string()];
        let env_check_arshy = |key: &str| key == "ARSHY_AGENT";
        assert!(!should_intercept(&args, env_check_arshy, &None, false));
        assert!(!should_intercept(&args, |_k: &str| false, &Some("node".to_string()), false));
        assert!(!should_intercept(&args, |_k: &str| false, &Some("python".to_string()), false));
    }
    ...
    fn test_should_intercept_known_agent_without_workspace() {
        ...
        for parent in [
            ...
            assert!(
                should_intercept(&args, env_check, &Some(parent.to_string()), false),
