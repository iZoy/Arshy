...
mod tests {
    ...
    fn default_config_compiles() {
        ...
        assert!(!cfg.store.store_dir.to_string_lossy().is_empty());
    }
    ...
    fn default_config_all_sections() {
        ...
        // MCP
        // McpConfig is intentionally empty; mcp-level knobs are not exposed yet.
    }
    ...
    fn expand_path_with_xdg_data_home() {
        ...
        let result = expand_path(&PathBuf::from("${XDG_DATA_HOME}/arshy/store"));
        assert!(result.to_string_lossy().starts_with(&xdg));
        assert!(result.to_string_lossy().ends_with("arshy/store"));
    }
    ...
    #[test]
    fn expanded_store_dir_is_absolute() {
        let cfg = Config::default();
        let expanded = cfg.store.expanded_store_dir();
        let s = expanded.to_string_lossy();
        assert!(!s.contains("${"), "path still has placeholder: {}", s);
    }
}
