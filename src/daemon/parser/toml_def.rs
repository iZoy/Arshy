...
pub struct MetaDef {
    ...
    /// When true, the whole parser is deprecated (prefer an alternative).
    #[serde(default)]
    pub deprecated: bool,
    /// Name of the replacement parser/approach for a deprecated parser.
    #[serde(default)]
...
impl PatternDef {
    ...
    fn to_line_pattern(&self) -> Result<LinePattern, String> {
        ...
        Ok(LinePattern {
            ...
            severity_group: self.fields.get("severity").copied(),
            ...
            match TomlParserDef::parse(content) {
                Ok(def) => {
                    if def.meta.deprecated {
                        tracing::warn!(
                            "builtin parser '{}' is deprecated{}",
                            def.meta.name,
                            def.meta
                                .replaced_by
                                .as_ref()
                                .map(|r| format!(", prefer: {}", r))
                                .unwrap_or_default()
                        );
                    }
                    Some((name, def))
